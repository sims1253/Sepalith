#!/usr/bin/env python3
"""PFT1 probe prep: merge the BANKED b4 LoRA onto the GDN base (CPU).

Same flow + gates as scripts/b8b_merge_base.py (export_gguf.py's
MERGE_VIA_PEFT branch with the B8b merge gates G1-G3), pointed at the
banked b4 anchor adapter — the BPB forgetting probe needs the anchor as
FULL weights to pair against the full-FT arm on identical bytes/tokens.
Output PERSISTENT (not /tmp). CPU-only (no CUDA context; card discipline).
Run under .venv-sft.
"""
import json
import sys
...
LORA = Path("/mnt/h/sepalith/runs/b4_qwen35_2b/final_lora")
OUT = Path("/mnt/h/sepalith/runs/pft1_b4_merged")
...
print(f"OK -> {OUT} (PFT1 b4 anchor merged, probe-ready)", flush=True)
