#!/usr/bin/env python3
"""B10 WiSE-FT dose-response on the b4 production pick; no automatic adoption.

Arms (all Q8_0, quantized by the pinned quantizer from frozen f16 parents):
  b4      alpha=1.0 anchor (the production recipe artifact)
  base    alpha=0.0 (the resolved base pick, untrained)
  a30/a50/a70  WiSE-FT interpolations
Frozen 513-case cohort (255 scenario + 258 no-op), identical prompts/stops/limits
per arm; paired McNemar vs the b4 anchor within this run only.
"""
...
ARMS = ['b4', 'base', 'a30', 'a50', 'a70']
ALPHA = {'b4': 1.0, 'base': 0.0, 'a30': 0.3, 'a50': 0.5, 'a70': 0.7}
...
def prepare(run, assets):
    ...
    def digest(p):
        ...
        with open(p, 'rb') as f:
            for b in iter(lambda: f.read(8 << 20), b''):
                h.update(b)
            return h.hexdigest()
    am = json.loads((assets / 'assets-manifest.json').read_text())
    model_sha = {arm: am['quant_receipts'][arm]['output_sha256'] for arm in ARMS}
    if not am.get('b4_matches_banked'):
        raise ValueError('assets manifest did not verify the banked b4 anchor')
    noop_split = {}
    for r in rows:
        if r['kind'] == 'noop':
    ...
    write(run / 'prepared.json', dict(
        ...
        expected_rows=513 * len(ARMS), ctx=8192, max_seconds=5400,
...
def evaluate(run):
    ...
    for arm in p['arms']:
        ...
        summary[arm] = dict(alpha=p['alphas'][arm], scenarios=len(sc), exact=sum(r['exact'] for r in sc), valid=sum(r['valid_pass'] for r in sc),
    ...
    write(run / 'evaluation.json', dict(rows=len(rows), dose_response=dose, arms=summary, paired_vs_b4=comparisons,
                                        statistics_note='12 unadjusted exact-McNemar tests (4 arms x 3 metrics), exploratory: '
