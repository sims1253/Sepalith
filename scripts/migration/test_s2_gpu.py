...
def fixture_run(path):
    s2.write(path/'prepared.json',dict(formats=['Q8_0','Q4_0'],trace_ids=['a'],reps=1))
    rows=[dict(format=fmt,rep=0,trace_id='a',prompt_n=2000,tokenized_prompt_tokens=2000,
          raw_response=dict(stop=True,stop_type='word'),prompt_ms=100,predicted_ms=ms,wall_ms=ms+105)
