...
def fixture_run(path,mismatch=False,drafting=True):
    ids=['a2','b2','a8','b8'];arms=['baseline','ngram-simple@16']
    ...
    rows=[dict(arm=arm,trace_id=tr,ctx_class='2k' if tr.endswith('2') else '8k',gen_tps=100 if arm=='baseline' else 120,
          draft_n=10 if drafting else 0,error=None,stop_hit=True,n_prompt_srv=100,tokenized_prompt_tokens=100,matches_baseline=not mismatch,
...
def test_speed_without_drafting_does_not_nominate(tmp_path):
    fixture_run(tmp_path,drafting=False);s.evaluate(tmp_path)
    assert not json.loads((tmp_path/'verdict.json').read_text())['confirmation_candidates']
