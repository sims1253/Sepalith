...
def main():
 ...
 quality=json.loads((a.owner/'b4-other-quants.recipe.json').read_text())
 ...
 for item in quality['inputs']+traces_recipe['inputs']:
  path=Path(item['path'])
  if path.name in ('Q8_0.gguf','Q6_K_imatrix.gguf','IQ4_XS_imatrix.gguf','traces.jsonl'):
 ...
 assets,common=freeze(a.owner/'assets-b4-other-timing',files)
 includes=['packages/sepalith/src','experiments/eval/spec_bench.py','scripts/migration/s1_gpu.py',
           'scripts/migration/b4_other_timing.py','scripts/migration/v1c_artifacts.py']
 snapshot=runner.snapshot(ROOT,includes)
 reviews=[json.loads((a.owner/'pi-b4-other-timing'/(provider+'.json')).read_text()) for provider in ('opencode','zai')]
 for tier,previous in [('gpu',quality),('cpu',cpu)]:
  ...
  for action,artifacts in [('prepare',['prepared.json']),('measure',['per_request.jsonl','telemetry.jsonl','warmups.jsonl']),('evaluate',['evaluation.json','verdict.json'])]:
   recipe['steps'].append(dict(id=action,argv=['{python}','{source}/scripts/migration/b4_other_timing.py',action,'--run','{run}','--assets',str(assets)],artifacts=artifacts))
  ...
  output=a.owner/('b4-other-timing-'+tier+'.recipe.json')
