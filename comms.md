...
| zcode-b9-select | B9 SeleKT gradient-importance masking A/B (queue §2b B9; winner track's last open item; build → validated run → battery) | subagent of zcode-queue-mgr-2 | active 2026-09-06 (build validated: selekt_data.py + train_sft.py SELEKT mode default-OFF, 27 B9 tests + 31 B8 regression green; firing the arm) |
