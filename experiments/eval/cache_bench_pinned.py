...
Writes experiments/eval/results_cachebench_pinned.jsonl.
"""
import json
import os
import signal
...
OUT = HERE / "results_cachebench_pinned.jsonl"
SUFFIX_LINES = 130
GEN = 16
PORT = 18098
...
def main():
    ...
    log = open("/tmp/a2gates/llama-server-cachebench-pinned.log", "ab")
    proc = subprocess.Popen(
        [str(SERVER), "-m", str(MODEL), "--port", str(PORT), "--host",
         "127.0.0.1", "-t", "6", "-ngl", "0", "--parallel", "1",
         "-c", "16384", "--cache-reuse", "256"],
