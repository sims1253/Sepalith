...
- **B10 WiSE-FT — DONE 2026-09-09 01:5x: NO-ADOPT on the product axis (dose-response banked; BPB recovery leg queued)** — attempt f52917a8b54b4f9995fcdc06938cda06, 2565 rows, frozen 513-case
  cohort x {b4 anchor, base, alpha 0.3/0.5/0.7}, all Q8 from frozen f16 parents (fresh b4 Q8 reproduced the
  banked export byte-for-byte). exact/valid: 196/217 -> 184/206 (a70, p=0.012/0.035) -> 170/184 (a50) ->
  101/116 (a30) -> 0/0 (base); noopFP ties b4 at alpha>=0.5 (structural floor confirmed adapter-scale-invariant);
  62% generation-limit rate on base = stops are fully trained. Scope correction vs runbook documented: TRUE
...
Last synced: 2026-09-06T13:0x+02 (zcode-ml-intake — PROPOSED intake, user-requested; status edits
only, nothing fired: ML-series (ML1/ML2/ML3/MLV/MLK) added to §3 — Matryoshka on
the COMPUTE axis (loop-count levels over one shared trunk) + EBT-verifier lane,
from `docs/research/2026-09-06-looped-matryoshka-plan.md`; P13-S1 fold noted
in-premise; params-axis "Matryoshka draft" (S1 arm) disambiguated; board post
appended.)
...
| B11 | NextCoder-style R edit-seed pack — **v1 PARTIAL DONE 2026-09-09: 973 rows assembled** (28% of task space; provider rate-quota blocked; resume-safe top-ups when quota resets; class mix tidyverse-heavy noted as v2 mining lever; idiom+operator gates + per-row provenance; docs/validation/2026-09-09-b11-nextcoder-r-v1.json) | API+CPU, anytime | The edit-shaped data construction for post-train v1 | runbook §2 B11 + experiments/data-mining/build_nextcoder_r.py |
...
| O2 | **Unanimous-group guard (pass-rate filter)** — **DONE 2026-09-09: MECHANISM-CONFIRMED, PRODUCT-AXIS TIE-UNDERPOWERED; WINNER-FILTER NOT ESTABLISHED at 300 steps** (screen: 5,578 prompts k=16 under frozen b4, 1,592 admitted, floor guard tripped -> documented matched-budget rescale to 942 quota-draw prompts; arms: only delta = admission filter, 300 steps seed 3407 on merged b4/sft_v7; first50 psg 0.660 vs 0.3175 = >2x variance, within-family confirmed; battery: exact 206 vs 202 p=0.289, valid 228 vs 223 p=0.125, noop identical 0/0 discord; admission ledger banked as standing instrument — rl_smoke --prescreen with fail-closed preflight; filter = design option for W17's longer RL phase, not default adoption; evidence docs/validation/2026-09-09-o2{,-prescreen,-verdict}.json + r1-r6 failed-attempt records; serving-path finding: llama-server b10453 rejects invalid-UTF-8 temp-1.0 rollouts via Content-only PEG on ALL endpoints — HF rollout path is the only faithful one at temp 1.0)
...
### ML-series — Matryoshka on the compute axis + EBT verifier (user thread 2026-09-06; intake 13:0x, PROPOSED)

Premise: the EBT thread (Gladstone/He arXiv 2507.02092, ICLR'26) and the
user's looped-transformer-for-the-largest-level plan converge into one
design: **Matryoshka levels = loop count over ONE shared trunk** —
ML-L1/L2/L4 = 1/2/4 Δ-gated refinement passes (`h_t = h_t-1 + g_t ⊙
(Trunk(h_t-1) − h_t-1)`, zero-init g ⇒ every level is bit-exact ML-L1 at
init → function-preserving growth, banked-verdict pinning). Heterogeneous
*computation* per level, homogeneous *weights* — the cheap honest version
of "different architectures per level". **NOT the params-axis Matryoshka**
(S1's `--model-draft` arm — separate weights, stays S1's). **Folds P13**:
ML rung-1 IS P13-S1's looped-vs-compute-matched arms promoted to 206M +
the random-L training mix + level gates; P13-S0 keeps the SERVING half of
the loop verdict, ML-G4 takes the QUALITY half — one GPU spend, two
verdicts. Generative-EBT route REJECTED on cost math (3.3–6.6× per-step
FLOPs, ~10×-to-same-ppl per the DL.ai replication; 400M-class generative
EBT ≈ week-plus exclusive on this box) — the EBT survivor is the verifier
lane (MLV). Runbook = `docs/research/2026-09-06-looped-matryoshka-plan.md`
(design §1–2, budget table §3, gates G0–G5 §4, kernel rules §6, ops §7).

| # | Experiment | Class / cost | Why (the datapoint) | Entry point |
|---|---|---|---|---|
| ML1 | **Loop-mix rung-1 — the cheap verdict** — 206M ladder trunk, random-L mix {1: 0.5, 2: 0.25, 4: 0.25} (L̄=2.0), 0.25B tokens = 0.5B trunk-passes = FLOP-matched **by design intent** vs an AR 206M@0.5B control — **CORRECTED 2026-09-08 (research-lead triage): the prior 'banked D1 = AR control, zero spend' claim is FALSE (docs/RESEARCH-AUDIT.md finding 1: D1 trained masked diffusion, different vocab); ML1 needs a fresh AR control arm (~+2h GPU) and a written FLOP ledger before any match claim**. Gates: G0 health + gate-telemetry (silent-collapse watch: g pinned at 0 ⇒ all levels ≡ L1 and everything below vacuously passes) · G2 level-1 innocence ≤+1% BPB vs AR control (FIM-Replica band) · G3 anytime monotone, ≥0.5% per doubling · **G4 WINNER-LOOP iff ≥ parity FLOP-matched AND G3 holds; loss >1% ⇒ KILL the generative loop line** (loop then lives/dies with P13's serving math alone). Supersedes P13-S1's B/C-vs-D spend if both GO | ~2h GPU, serial per W37, behind the §1 chain | The comparison the EBT paper skipped (equal-steps-not-FLOPs under a 3.3–6.6× step cost) run honestly at our scale; ~2h decides whether the line earns its 26h confirmation rung | plan §2–4 + `poc_twin` ladder rig + D1 banked row |
| ML2 | **Growth rung (incremental-Matryoshka experiment proper)** — continue ML1 → K_max=8 via fresh zero-init gates; **G1 = grown-init outputs bit-exact vs pre-growth at L=1** (seam-exact class, B8 precedent); old params 0.3× LR (per-group, PFT1 env-knob pattern); per-level frozen snapshot exports MANDATORY (old levels drift post-growth — expected, that is why snapshots) | ~2-3h GPU, behind ML1-win | The property that lets a compute-axis Matryoshka grow level-by-level without orphaning banked artifacts | plan §2 + ML1 ckpt |
| ML3 | **2B-token confirmation pair** — looped 2B×L̄2 vs fresh AR@4B arm (4B trunk-passes each side) | ~26h pair, GO only on ML1 G4-win | Rung-1's verdict at 8× budget — separates small-scale artifact from scaling behavior before anything A2-adjacent gets said | plan §3 |
| MLV | **EBT verifier v1** — NCE contrastive energy head over (prompt, completion) on judge/pairwise-pref infra data (V1D labels), head-MRL nesting {128/256/512} on the pooled rep, **NO inner loop in v1** (feedforward scorer; K-step latent refinement = conditional v2). Readouts: chosen-vs-rejected AUROC (**kill < 0.65**), pairwise-pref accuracy vs V1D labels, ECE, coarse-to-fine routing win (small-nest prefilter + full verify vs full-only at matched avg FLOPs — noopFP-abstain adjacent, X5-S0 instrument sibling) | ~4-8h GPU; independent of ML1–3, slots anywhere behind the chain | The EBT thread's surviving artifact: verifier-as-energy without the generative cost; plugs into the existing judge infra on day one | plan §5 + `experiments/eval/V1D_RESULTS.md` + `pairwise_pref.py` |
| MLK | **Kernel side-lane (standing item, no GPU claim of its own)** — golden tests vs `torch.compile` ref + fixed tolerance gates (PFT1 byte-identical-default discipline), perf leaderboard file, post-experiment windows ONLY (W37 as broadened), ~5-min rounds. **RULE: verdicts are FLOP-matched, not wall-clock-matched — no kernel work gates any G-gate** (ddot frozen-optimization precedent: graphed 30.4k < 35k ⇒ reverted) | spare-quota class | Loop iterations = same-shape repeated work over resident weights = the persistent/fused-kernel-friendly regime (`graphed_step.py` 1.7×/micro in-house precedent; ~50 eff-TFLOPs anchor leaves ~2× headroom) — the throughput multiplier for whichever arm survives, never a prerequisite | plan §6 + `poc_ddot/graphed_step.py` |

Order: ML1 (cheap verdict first, banked control) → ML2 behind an ML1 win
→ ML3 only on a G4 win; MLV independent, slots behind the current chain;
MLK continuous, never on the critical path. Interactions: P13 fold
(premise), PFT1 (per-group LR precedent for growth), D-grid (banked
control), X5 (instrument cross-pollination; lines stay separate).
...
- 2026-09-06 (17:0x): **SESSION CLOSE-OUT — production recipe RESOLVED, plan
  v1 committed** (`2026-09-04-production-finetune-plan.md` v1: base b4-config
  by elimination / data sft_v7 raw / NO midtrain / plain SFT / LoRA r32α64 /
  LR 2e-4 sweep-validated; RL LoRA-on-LoRA-base; risk register incl.
  domain-asymmetric forgetting + X5 abstain-gate lever). Session totals
  (~42h): 22 verdicts/preps — B13 tie+elimination (base pick), B8/B8b
  (midtrain dropped), B9 (masking closed), PFT1 (LoRA clean sweep), E1/O1
  KILLED, TU1 DEAD + TU2 raw-route-stands, O3-S0 NOT LAND, V1d ALIVE, H1
  hill-climb+H2-moot, P12 GO, FIM-Replica falsifier PASSED, P10 GN
  structural/GN-v2-dominates, X5-S0 pass + X5-S1 NO-WIN with bankable
  abstain instrument (44.5-80.3% @0-lost), S0 traces + S1 rig + W13 serve
  path, LOC1-S0 gate-pass/lexical-tie, LR sweep (2e-4 stands), dashboard v2
  spark-designed ACCEPTED. Spend: ~$4 of $100 Anyscale (reserve ~$85);
  Kaggle 0.75/30 GPU-h used, GPU vehicle blocked upstream (unsloth
  no-bf16-on-pre-Ampere; retry conditions in the integration doc), $100/mo
  inference credits earmarked for the benchmark-scan thread. IN FLIGHT at
  close: the S1/S2/V1c bench chain (ETA ~03:30; stamps owed: S1 row, S2
  ship-matrix, V1c eval-v2 tail; S3-S0 unlocks on S1's legs). UNTRIAGED by
  this session: the ML-series (below, 13:0x) → successor's first triage.
  USER DECISIONS PARKED: benchmark staged-GO + 50-pkg carve + naming;
  LOC1-S1 ($50-100). NEXT SET: W16 (live head), comparable production run,
  H3-S0, TU3 build, S3 gate, O2 filter arm, ML-series triage.

- 2026-09-06 (13:0x): added ML-series (ML1/ML2/ML3/MLV/MLK — Matryoshka on
  the COMPUTE axis + EBT verifier lane) to §3 PROPOSED per user request,
  from the 2026-09-06 EBT/looped-transformer thread; plan pre-registered
  `docs/research/2026-09-06-looped-matryoshka-plan.md`. ML1 = loop-mix
  rung-1 (206M, random-L {1,2,4} L̄=2, 0.25B tok = FLOP-matched by
  construction vs banked D1, ~2h; gates G0–G4, kill at >1% FLOP-matched
  loss). ML2 = growth rung (K_max=8, bit-exact G1, 0.3×-LR old params,
  per-level snapshots mandatory). ML3 = 26h 2B-token confirmation pair,
  G4-win only. MLV = EBT verifier v1 (NCE energy head on V1D/judge data,
  head-MRL nesting, AUROC ≥0.65 gate, no inner loop in v1; generative-EBT
  rejected on cost math — premise note). MLK = kernel side-lane
  (FLOP-matched verdicts never gate on kernels; post-experiment windows
  per W37-broadened). Disambiguation: NOT the params-axis Matryoshka
  draft (S1's `--model-draft` arm); P13-S1's B/C-vs-D spend folds into
  ML1 if both GO. Announced on the board. No GO asked, nothing fired.
